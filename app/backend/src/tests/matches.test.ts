...
import { matchesFinishedsMock, matchesInProgressMock, matchesMock, newMatchMock, sucessMatchMock } from './mocks/matches.mock';
...
describe('Testes da seção 3', () => {
  ...
  describe('Tests na rota /matches', () => {
    ...
    it('Testa se criou uma partida', async () => {
      ...
      sinon.stub(Match, 'create').resolves(sucessMatchMock as any);

      chaiHttpResponse = await chai.request(app).post('/matches').send(newMatchMock).set('authorization', 'token');
    ...
    it('Falha se um time não existir', async () => {
      ...
      sinon.stub(jwt, 'verify').returns({ id: 12 } as any);
      chaiHttpResponse = await chai
      .request(app)
      .post('/matches')
      .send({
